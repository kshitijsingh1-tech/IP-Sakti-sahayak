...
export const AgentPipeline: React.FC<AgentPipelineProps> = ({ steps }) => {
  ...
  return (
    <div className="rounded-3xl bg-white p-6 border border-slate-200 shadow-lg relative overflow-hidden">
      <div className="flex items-center justify-between mb-4 border-b border-slate-200 pb-3">
        ...
        <span className="text-[11px] font-mono px-3 py-1 rounded-full bg-slate-950 text-white font-bold">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 relative">
        {steps.map((step, index) => {
          ...
          return (
            <div
              key={index}
              className={`p-4 rounded-2xl border transition-all duration-300 flex flex-col justify-between ${getAgentCardStyle(
                isRunning,
                isDone
              )}`}
            >
              <div>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-black font-display uppercase tracking-wider text-slate-950">
                ...
                <strong className="text-xs font-black block mb-1 text-slate-950">{step.title}</strong>
                <p className="text-[11px] text-slate-600 mb-3 line-clamp-2 font-medium">{step.details}</p>

                {/* Findings Checklist */}
                {isDone && step.findings && (
                  <div className="space-y-1 pt-2 border-t border-slate-200">
                    {step.findings.map((finding, fIdx) => (
                      <div key={fIdx} className="text-[10px] flex items-start gap-1 text-slate-800 font-medium">
              ...
              {/* Status Footer Badge */}
              <div className="mt-3 pt-2 border-t border-slate-200 text-[10px] font-mono flex items-center justify-between font-bold">
